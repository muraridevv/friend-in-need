...
class MemoryServiceTest {
    @Test
    void usesLexicalFallbackWhenEmbeddingIsUnavailable() {
    ...
    @Test
    void delegatesVectorSimilarityToDatabase() {
        CompanionMemoryRepository repository = mock(CompanionMemoryRepository.class);
        EmbeddingService embeddings = mock(EmbeddingService.class);
        UUID profileId = UUID.randomUUID();
        when(embeddings.embed("travel")).thenReturn(new float[]{0.1f, 0.2f});
        when(repository.findSimilar(eq(profileId), anyString(), eq(4))).thenReturn(List.of());
