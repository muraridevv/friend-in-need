...
public class NotificationPublisher {
    ...
    public boolean publish(ProactiveMessage message) {
        ...
        try { emitter.send(SseEmitter.event().name("proactive").data(message.getContent())); return true; }
        catch (IOException exception) { emitters.remove(message.getProfileId(), emitter); emitter.completeWithError(exception); return false; }
    }
}
