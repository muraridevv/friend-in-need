...
public class ContextService {
    ...
    public ContextService(WeatherService weather, CalendarService calendar, NewsService news, ConsentService consent, SmartHomeAdapter home, TimerService timers) {
    ...
    public String relevantContext(CompanionProfile profile) {
        ...
        String homeContext = "";
        if (consent.isEnabled(profile.getId(), IntegrationType.SMART_HOME)) {
            homeContext = " Smart-home devices: " + home.listDevices() + ". Active timers: " + timers.list(profile.getId()).stream().filter(timer -> !timer.isFired()).map(timer -> timer.getLabel() + " at " + timer.getTriggerAt()).reduce((a, b) -> a + "; " + b).orElse("none") + ".";
        }
        return "Local time: " + ZonedDateTime.now(timezone) + ". " + weather.current(profile.getLocation()) + " Upcoming calendar: " + appointments + " News: " + headlines + homeContext;
    }
}
