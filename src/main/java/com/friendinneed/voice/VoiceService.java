...
public class VoiceService {
    ...
    private final LocalSttService localStt;
    private final LocalTtsService localTts;

    public VoiceService(VoiceProperties properties, ModelRouter modelRouter, LocalSttService localStt, LocalTtsService localTts) {
        ...
        log.info("Voice provider configured: baseUrl={}, sttEndpoint={}audio/transcriptions, ttsEndpoint={}audio/speech", baseUrl, baseUrl, baseUrl);
    }

    public String transcribe(MultipartFile audio) throws IOException {
        if (modelRouter.routeStt() == ModelChoice.LOCAL) {
            log.info("Routing STT to local provider");
            return localStt.transcribe(audio);
        }
        configured();
        log.info("Requesting STT transcription: model={}, contentType={}, sizeBytes={}", properties.sttModel(), audio.getContentType(), audio.getSize());
        ...
        log.info("STT transcription completed: textLength={}", text.length());
        return text;
    }
    ...
    public byte[] speak(String text, EmotionLabel emotion) {
        if (modelRouter.routeTts() == ModelChoice.LOCAL) {
            log.info("Routing TTS to local provider");
            return localTts.speak(text);
        }
        configured();
        log.info("Requesting TTS synthesis: model={}, voice={}, textLength={}", properties.ttsModel(), properties.ttsVoice(), text.length());
        return client.post().uri("audio/speech").header(HttpHeaders.AUTHORIZATION, "Bearer " + properties.apiKey()).contentType(MediaType.APPLICATION_JSON)
    ...
    private void configured() {
        if (properties.baseUrl() == null || properties.baseUrl().isBlank() || properties.apiKey() == null || properties.apiKey().isBlank())
            throw new IllegalStateException("Voice provider is not configured");
    }
