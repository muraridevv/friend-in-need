...
public class VoiceController {
    ...
    public VoiceController(VoiceService voice) { this.voice=voice; }
    @PostMapping(value="/transcriptions",consumes=MediaType.MULTIPART_FORM_DATA_VALUE)
    Map<String,String> transcribe(@RequestPart("audio") MultipartFile audio) throws IOException { return Map.of("text",voice.transcribe(audio)); }
    @PostMapping(value="/speech",produces="audio/mpeg")
    byte[] speech(@RequestBody SpeechRequest request) { return voice.speak(request.text()); }
