...
public class RoutineController {
    ...
    public RoutineController(RoutineService s, ConsentService c, CompanionProfileRepository p, UserRepository u) {
        service = s;
        consent = c;
        profiles = p;
        users = u;
    ...
    private void allowed(UUID p) {
        owner(p);
        if (!consent.isEnabled(p, IntegrationType.SMART_HOME))
            throw new org.springframework.security.access.AccessDeniedException("Smart home consent is required");
    }

    @GetMapping
    List<Routine> list(@PathVariable UUID profileId) {
        allowed(profileId);
    ...
    Routine create(@PathVariable UUID profileId, @RequestBody Request r) {
        allowed(profileId);
        return service.create(profileId, r.triggerPhrase(), r.steps());
    }
    ...
    record Request(String triggerPhrase, List<Map<String, Object>> steps) {
    }
}
